...
subList from to xs
    |from < 0 || to < from = []
    |otherwise = take elementsToTake (drop from xs)
    where elementsToTake = to - from + 1
...
smartReplicate = concatMap repeatValue
  where repeatValue x = replicate x x
{- |
=⚔️= Task 9
